...
| `COLLCT_MOMENTS_SCHEDULER` | `true` | In-process per-minute scheduler that fires moment notifications during the window. Docker/bare-Node need nothing else. Set `false` when an external cron owns the job. |
...
Native push senders are not implemented. The `platform` column on
`push_subscriptions` is left open so native clients can register with
`platform: 'ios'` / `'android'` today; those rows are stored and skipped
until a sender exists. No env vars.
